...
<body>
   ...
   <button id="addcatebtn">Order List</button>
    ...
    <div class="message">
        <c:choose>
            <c:when test="${resultFlag == 1}">
                <p class="success" style="margin-left:300px;color:blue;">Successfully delete order!</p><br>
            </c:when>
            <c:when test="${resultFlag == 0}">
                <p class="error" style="margin-left:300px;color:red;">Can't delete this order!!this order has orderdetal!!</p><br>
            </c:when>
        </c:choose>
    </div>
</c:if>
         ...
         <thead>
           <tr>
			...
			<tr>
				...
				<td>${order.date}</td>
				<td>${order.user_id}</td>
				<td><a href="deleteorder/${order.id}" id="ebbtn" onclick="return confirm('Are you sure Delete?')">Delete</a></td>
